...
__kernel void backCalcGradients(__global float *error, __global float *preActivations, __global float *gradients, int numOfPerceptrons, int weightsDim, int batchSize, int prevActDim)
{
    ...
    for (int i = 2 * groupSize_k1_M * gid0; i < numOfPerceptrons; i += 8192)
      for (int j = 2 * groupSize_k1_N * gid1; j < weightsDim; j += 8192) {
        for (int k = 0; k <= (batchSize >= 1 ? batchSize - 1 : 0); k += groupSize_k1_K) {
          for (int c0 = lid0; c0 <= min(groupSize_k1_K - 1, batchSize - k - 1); c0 += groupSize_k1_M)
            for (int c1 = lid1; c1 <= min(2 * groupSize_k1_N - 1, weightsDim - j - 1); c1 += groupSize_k1_N)
              if (weightsDim != prevActDim && j + c1 == weightsDim - 1)
                shared_B[c0][c1] = 1.0f;
              else
                shared_B[c0][c1] = preActivations[(j + c1) + (k + c0) * prevActDim];
          for (int c0 = lid0; c0 <= min(2 * groupSize_k1_M - 1, numOfPerceptrons - i - 1); c0 += groupSize_k1_M)
            for (int c1 = lid1; c1 <= min(groupSize_k1_K - 1, batchSize - k - 1); c1 += groupSize_k1_N)
          ...
          if (numOfPerceptrons >= lid0 + i + 1 && weightsDim >= lid1 + j + 1)
            for (int c2 = 0; c2 <= min(groupSize_k1_K - 1, batchSize - k - 1); c2 += 1) {
              private_C[0][0] += (shared_A[lid0][c2] * shared_B[c2][lid1]);
              if (weightsDim >= lid1 + j + groupSize_k1_N + 1)
                private_C[0][1] += (shared_A[lid0][c2] * shared_B[c2][lid1 + groupSize_k1_N]);
              if (numOfPerceptrons >= lid0 + i + groupSize_k1_M + 1) {
                private_C[1][0] += (shared_A[lid0 + groupSize_k1_M][c2] * shared_B[c2][lid1]);
                if (weightsDim >= lid1 + j + groupSize_k1_N + 1)
                  private_C[1][1] += (shared_A[lid0 + groupSize_k1_M][c2] * shared_B[c2][lid1 + groupSize_k1_N]);
              }
            }
          barrier(CLK_LOCAL_MEM_FENCE | CLK_GLOBAL_MEM_FENCE);
        }
                                          ...
                                          int batchSize, int prevActDim, int numOfPerceptrons, int weightsDim, __global float *prevActivations)
{
    ...
    float private_C[2][2];
    __local float shared_Act[2 * groupSize_k2_M][2 * groupSize_k2_N];

    for (int i = 2 * groupSize_k2_M * gid0; i < batchSize; i += 8192)
      for (int j = 2 * groupSize_k2_N * gid1; j < prevActDim; j += 8192) {
        // if (prevActDim >= j + 1)
        for (int c0 = lid0; c0 <= min(2 * groupSize_k2_M - 1, batchSize - i - 1); c0 += groupSize_k2_M)
          for (int c1 = lid1; c1 <= min(2 * groupSize_k2_N - 1, prevActDim - j - 1); c1 += groupSize_k2_N)
            shared_Act[c0][c1] = prevActivations[(i + c0) * prevActDim + (j + c1)];
        for (int k = 0; k <= (numOfPerceptrons >= 1 ? numOfPerceptrons - 1 : 0); k += groupSize_k2_K) {
          for (int c0 = lid0; c0 <= min(groupSize_k2_K - 1, numOfPerceptrons - k - 1); c0 += groupSize_k2_M)
            for (int c1 = lid1; c1 <= min(2 * groupSize_k2_N - 1, prevActDim - j - 1); c1 += groupSize_k2_N)
              shared_B[c0][c1] = weights[(k + c0) * weightsDim + (j + c1)];
          for (int c0 = lid0; c0 <= min(2 * groupSize_k2_M - 1, batchSize - i - 1); c0 += groupSize_k2_M)
            for (int c1 = lid1; c1 <= min(groupSize_k2_K - 1, numOfPerceptrons - k - 1); c1 += groupSize_k2_N)
              shared_A[c0][c1] = error[(i + c0) * numOfPerceptrons + (k + c1)];
          barrier(CLK_LOCAL_MEM_FENCE | CLK_GLOBAL_MEM_FENCE);
          if (k == 0 && batchSize >= lid0 + i + 1 && prevActDim >= lid1 + j + 1) {
            private_C[0][0] = 0;
            if (prevActDim >= lid1 + j + groupSize_k2_N + 1)
              private_C[0][1] = 0;
            if (batchSize >= lid0 + i + groupSize_k2_M + 1) {
              private_C[1][0] = 0;
              if (prevActDim >= lid1 + j + groupSize_k2_N + 1)
                private_C[1][1] = 0;
            }
          ...
          if (batchSize >= lid0 + i + 1 && prevActDim >= lid1 + j + 1)
            for (int c2 = 0; c2 <= min(groupSize_k2_K - 1, numOfPerceptrons - k - 1); c2 += 1) {
              private_C[0][0] += (shared_A[lid0][c2] * shared_B[c2][lid1]) * derivative(shared_Act[lid0][lid1]);
              if (prevActDim >= lid1 + j + groupSize_k2_N + 1)
                private_C[0][1] += (shared_A[lid0][c2] * shared_B[c2][lid1 + groupSize_k2_N]) * derivative(shared_Act[lid0][lid1 + groupSize_k2_N]);
              if (batchSize >= lid0 + i + groupSize_k2_M + 1) {
                private_C[1][0] += (shared_A[lid0 + groupSize_k2_M][c2] * shared_B[c2][lid1]) * derivative(shared_Act[lid0 + groupSize_k2_M][lid1]);
                if (prevActDim >= lid1 + j + groupSize_k2_N + 1)
                  private_C[1][1] += (shared_A[lid0 + groupSize_k2_M][c2] * shared_B[c2][lid1 + groupSize_k2_N]) * derivative(shared_Act[lid0 + groupSize_k2_M][lid1 + groupSize_k2_N]);
              }
            }
          barrier(CLK_LOCAL_MEM_FENCE | CLK_GLOBAL_MEM_FENCE);
        }
        if (prevActDim >= lid1 + j + 1&& batchSize >= lid0 + i + 1) {
          ...
          if (prevActDim >= lid1 + j + groupSize_k2_N + 1)
            prevError[(lid0 + i) * prevActDim + (lid1 + j + groupSize_k2_N)] = private_C[0][1];
          if (batchSize >= lid0 + i + groupSize_k2_M + 1) {
            prevError[(lid0 + i + groupSize_k2_M) * prevActDim + (lid1 + j)] = private_C[1][0];
            if (prevActDim >= lid1 + j + groupSize_k2_N + 1)
              prevError[(lid0 + i + groupSize_k2_M) * prevActDim + (lid1 + j + groupSize_k2_N)] = private_C[1][1];
          }
        }
        barrier(CLK_LOCAL_MEM_FENCE | CLK_GLOBAL_MEM_FENCE);
      }
